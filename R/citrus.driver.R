citrus.full = function(dataDir,outputDir,clusterCols,fileSampleSize,fileList,nFolds,featureTypes=c("densities"),transformCols=NULL,...){
  ...
  # Plot
  sapply(modelTypes,citrus.plotTypeErrorRate,outputDir=outputDir,regularizationThresholds=regularizationThresholds,thresholdErrorRates=thresholdErrorRates,thresholdFDRRates=thresholdFDRRates,cvMinima=cvMinima,thresholdSEMs=thresholdSEMs,foldModels=foldModels)
  ...
  # Plot Features
  lapply(modelTypes,citrus.plotDifferentialFeatures,differentialFeatures=differentialFeatures,foldFeatures=foldFeatures,outputDir=outputDir,labels=fileList[,"labels"])
